...
import zlib, re, sys, json
from PIL import Image, ImageDraw, ImageFont
TBL=bytes(b^0x64 for b in range(256))
OUT=r'C:\Users\truon\Desktop\GATE_DIASCREEN\HMI_OTL_RANG_iOS.dpa'
CAPS=json.loads(sys.argv[1]) if len(sys.argv)>1 else {}
OUTPNG=sys.argv[2] if len(sys.argv)>2 else 'preview_rang.png'
...
def hidden(el):
    vv=g(el,'VisibleVar','None') or 'None'
    m=re.search(r'W(\d+)', vv)
    return bool(m) and CAPS.get(m.group(1), 1) == 0
for el,st in items:
