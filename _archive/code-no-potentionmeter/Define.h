...
// Định nghĩa các cổng Serial tùy thuộc vào phiên bản V300 hoặc V400
#ifdef V300
#define SerialBluetooth Serial3 // Cổng Serial cho Bluetooth
#define SerialModbus    Serial4 // Cổng Serial cho Modbus
#define SerialHmi       Serial1 // Cổng Serial cho HMI
#define SerialComputer  Serial2 // Cổng Serial cho máy tính
#else
#define SerialBluetooth Serial3 // Cổng Serial cho Bluetooth
#define SerialModbus    Serial2 // Cổng Serial cho Modbus
#define SerialHmi       Serial1 // Cổng Serial cho HMI
#define SerialComputer  Serial4 // Cổng Serial cho máy tính
#endif

// Khai báo các đối tượng ModbusMaster để giao tiếp với các thiết bị qua giao thức Modbus RTU
ModbusMaster nodeBT;   // Đối tượng ModbusMaster cho cảm biến nhiệt độ BT
ModbusMaster nodeET;   // Đối tượng ModbusMaster cho cảm biến nhiệt độ ET
ModbusMaster nodeHMI;  // Đối tượng ModbusMaster cho giao tiếp với HMI
ModbusMaster nodeAir;  // Đối tượng ModbusMaster cho biến tần Airflow
ModbusMaster nodeDrum; // Đối tượng ModbusMaster cho biến tần Drum
...
//Array
// Khai báo các mảng và biến liên quan đến HMI và dữ liệu chương trình
uint16_t iMemHMI[45], iMemHMI_CP[45]; // Bộ nhớ nội bộ HMI ($M HMI)
uint16_t dAddress[200], dAddress_CP[200]; // Địa chỉ dữ liệu HMI (40xxx HMI)
uint8_t cAddress[200], cAddress_CP[200]; // Địa chỉ cuộn (Coils)
uint8_t progStatus = 0; // Trạng thái chương trình
uint16_t profileData[30]; // Dữ liệu hồ sơ chương trình
uint16_t profilePhaseHMI_R[10]; // Các giai đoạn cài đặt từ HMI
uint16_t dataConvert[30]; // Dữ liệu chuyển đổi

//Time
long timeMillis, calTime; //Tính thời gian xử lí chương trình
long timeLoaderMillis, loaderCalTime; //Tính thời gian xử lí chương tr

// Các biến thời gian liên quan đến chương trình rang cà phê
uint8_t minRoast = 0, secRoast = 0; // Phút và giây của thời gian rang
uint16_t timeRoast = 0;             // Tổng thời gian rang
uint16_t chargeTimer = 0;           // Bộ đếm thời gian cho quá trình charge
uint16_t dropTimer = 0;             // Bộ đếm thời gian cho quá trình drop
uint16_t coolTimer = 0;             // Bộ đếm thời gian cho quá trình làm mát
uint16_t escapeTimer = 0;           // Bộ đếm thời gian cho quá trình escape
uint16_t feederTimer = 0;           // Bộ đếm thời gian cho quá trình feeder
uint16_t destonerTimer = 0;         // Bộ đếm thời gian cho quá trình destoner
uint16_t abTimer = 0;               // Bộ đếm thời gian cho quá trình afterburner
uint16_t buzzerTimer = 0;           // Bộ đếm thời gian cho buzzer
uint16_t timerLimit = 3000;         // Giới hạn thời gian tối đa
uint16_t waitDropcloseTi = 0;       // Bộ đếm thời gian chờ đóng drop
uint8_t drumHzTimer = 0;            // Bộ đếm thời gian cập nhật Drum Hz
uint8_t airHzTimer = 0;             // Bộ đếm thời gian cập nhật Airflow Hz
uint8_t gasHzTimer = 0;             // Bộ đếm thời gian cập nhật Gas
uint8_t updateNetWTi = 0;           // Bộ đếm thời gian cập nhật Net Weight
uint8_t cleanFeederTi = 0;          // Bộ đếm thời gian làm sạch feeder
uint8_t delCyFeederTi = 0;          // Bộ đếm thời gian xóa chu kỳ feeder
...
uint16_t clRangeBt = 6; //Clearange để auto gas
...
uint8_t dif = 0;  //Căn chỉnh độ lệch khi cân

//----------------------Define profile data

// Nhiệt độ
#define BT_CHARGE_SAVE          profileData[0]  // Nhiệt độ Charge
#define BT_TP_SAVE              profileData[1]  // Nhiệt độ Turning Point (TP)
#define BT_YELLOW_SAVE          profileData[2]  // Nhiệt độ Yellow
#define BT_FCS_SAVE             profileData[3]  // Nhiệt độ First Crack Start (FCS)
#define BT_DROP_SAVE            profileData[4]  // Nhiệt độ Drop

// Thời gian
#define TIME_TP_SAVE            profileData[5]  // Thời gian Turning Point (TP)
#define TIME_TP_MIN_SAVE        profileData[6]  // Phút Turning Point (TP)
#define TIME_TP_SEC_SAVE        profileData[7]  // Giây Turning Point (TP)

#define TIME_YELLOW_SAVE        profileData[8]  // Thời gian Yellow
#define TIME_YELLOW_MIN_SAVE    profileData[9]  // Phút Yellow
#define TIME_YELLOW_SEC_SAVE    profileData[10] // Giây Yellow

#define TIME_FCS_SAVE           profileData[11] // Thời gian First Crack Start (FCS)
#define TIME_FCS_MIN_SAVE       profileData[12] // Phút First Crack Start (FCS)
#define TIME_FCS_SEC_SAVE       profileData[13] // Giây First Crack Start (FCS)

#define TIME_DROP_SAVE          profileData[14] // Thời gian Drop
#define TIME_DROP_MIN_SAVE      profileData[15] // Phút Drop
#define TIME_DROP_SEC_SAVE      profileData[16] // Giây Drop

#define PER_DEV_SAVE            profileData[17] // Phần trăm Development
#define TIME_DEV_MIN_SAVE       profileData[18] // Phút Development
#define TIME_DEV_SEC_SAVE       profileData[19] // Giây Development

#define TIME_YEL_SEC_GUE        profileData[20] // Dự đoán giây Yellow
#define TIME_FCS_SEC_GUE        profileData[21] // Dự đoán giây First Crack Start (FCS)

#define TIME_DEV_SAVE           profileData[22] // Thời gian Development
...
String STEP_AIRFLOW_WRITE = "NONE";
String STEP_GAS_WRITE = "NONE";
...
//ror calculate
SimpleKalmanFilter rorBTKalmanFilter(1, 1, 0.01);
SimpleKalmanFilter rorETKalmanFilter(1, 1, 0.01);
...
#define destonerValue_R     iMemHMI[destonerValue_W]
...
#define destonerValue_R_CP      iMemHMI_CP[destonerValue_W]
...
#define LOCK_AIR_DRUM_BURNER_W  8 //Lock Air/Drum/Burner
...
uint16_t sdTiRoast[3000];
uint16_t sdBT[3000];
uint16_t sdET[3000];
uint16_t sdAirflow[3000];
uint16_t sdGas[3000];
uint16_t sdDrum[3000];
int16_t sdRorBT[3000];
