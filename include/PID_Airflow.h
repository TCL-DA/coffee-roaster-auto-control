...
const float AIR_DEADBAND    =  3.0f;   // +/-3 Pa - vung khong dieu chinh (R1: chu thich cu ghi sai +/-5)
...
// FF Table
const int   FF_MAX_ENTRIES  = 60;   // R3: phai chua noi so buoc factory tune (51 buoc)
...
const int   FT_AIR_STEP      =  2;     // R3: 0->100 buoc 2 = 51 buoc, vua trong FF_MAX_ENTRIES=60
...
int      ffMapSize  = 0;
bool     ffDirty    = false;
bool     ffTableFull = false;  // R3: bang day giua luc quet -> Program.h bao ma loi
...
int ffFind(float sp) {
    int   best     = -1;
    float bestDist = FF_SP_MATCH;   // R2: truoc day +1.0f nen nhan ca entry cach 3,999 Pa
...
void ffLearn(float sp, float air) {
    ...
    if (i < 0) {
        if (ffMapSize < FF_MAX_ENTRIES) {
            ...
            // R3: TRUOC DAY im lang bo qua -> factory tune bao xong 100% nhung bang
            // thieu han vung gio cao, ffLookup roi ve uoc luong tuyen tinh -> snap sai.
            ffTableFull = true;
            if (enDebug) SerialComputer.println("FF FULL: entry bi bo, bang da day!");
        }
        return;
    }
